 ...
 */
export async function generateText(prompt, systemPrompt, provider) {
 ...
 */
export async function generateCover(prompt) {
    if (openai) {
        try {
            ...
            console.warn("[AI] DALL-E failed:", err.message);
        }
    }
...
function applyMockEdit(text, mode, instruction) {
...
// ─── Price suggestion ────────────────────────────────────────────────────────
export async function suggestPrice(title, description, wordCount) {
    const fallback = {
    ...
    try {
        const result = await generateText(`Title: "${title}"\nDescription: "${description}"\nWord count: ${wordCount}`, `You are a pricing assistant for digital guides sold on the Russian market.
...
Consider word count, topic complexity, and market rates for digital content in Russia.
      `);
        const json = extractJson(result);
        const parsed = JSON.parse(json);
        if (parsed.min && parsed.max && parsed.recommended)
            return parsed;
