...
router.post("/create", authMiddleware, async (req, res) => {
    try {
        ...
        const configuredReturnUrl = !configuredReturnUrlRaw ||
        ...
        const yookassaConfigured = Boolean(process.env.YOOKASSA_SHOP_ID?.trim() && process.env.YOOKASSA_SECRET_KEY?.trim());
        ...
        let payment;
        try {
            payment = await createPayment(Number(guide.price), "RUB", `Guide: ${guide.title}`, returnUrl);
        }
        catch (err) {
...
router.get("/order/:id", optionalAuthMiddleware, async (req, res) => {
    try {
