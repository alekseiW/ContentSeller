...
router.post("/structure", async (req: Request, res) => {
  try {
    const { text } = req.body;
...
router.post("/improve", async (req: Request, res) => {
  try {
...
router.post("/edit-selection", async (req: Request, res) => {
  try {
    const { text, mode, instruction } = req.body as {
      text?: unknown;
      mode?: unknown;
      instruction?: unknown;
    };
    ...
    const result = await editSelectionText(
      text,
      typeof mode === "string" ? mode : "improve",
...
router.post("/suggest-price", async (req: Request, res) => {
  try {
...
router.post("/generate-content", async (req: Request, res) => {
  try {
    const { prompt, context, maxLength } = req.body as {
      prompt?: unknown;
      context?: unknown;
      maxLength?: unknown;
    };
...
router.post("/cover", async (req: Request, res) => {
  try {
