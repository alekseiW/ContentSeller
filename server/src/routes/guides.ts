...
async function generateUniqueSlug(authorId: string, baseTitle: string): Promise<string> {
...
function mapGuideUpdates(input: Record<string, unknown>): Record<string, unknown> {
...
function redactPaidBlocksFromHtml(html: string): string {
...
function redactPaidInlineHtml(html: string): string {
  return html.replace(
    PAID_INLINE_REGEX,
    '<span data-paid-text="true" class="gh-paid-inline-placeholder">Paid content</span>',
  );
}

function redactInlinePaidContent(content: unknown): unknown {
...
function buildLockedSectionContent(): Record<string, unknown> {
  return {
    version: 2,
    format: "tiptap",
...
router.patch("/:id", authMiddleware, async (req: Request, res) => {
  try {
    ...
    const guideUpdates = mapGuideUpdates(rawGuideUpdates as Record<string, unknown>);
...
router.get("/catalog", async (req: Request, res) => {
  try {
...
router.get("/author/:username", optionalAuthMiddleware, async (req: Request, res) => {
  try {
...
router.get("/:username/:slug/public", optionalAuthMiddleware, async (req: Request, res) => {
  try {
    const username = String(req.params.username);
    ...
    for (const section of sections) {
      ...
      if (typeof s.content === 'object' && s.content !== null) {
        ...
        s.content = redactPaidBlocksFromHtml(s.content as string);
      }
    }
...
router.get("/:id/sections", authMiddleware, async (req: Request, res) => {
  try {
    const id = String(req.params.id);
...
router.post("/:id/sections", authMiddleware, async (req: Request, res) => {
  try {
    const id = String(req.params.id);
...
router.patch("/sections/:sectionId", authMiddleware, async (req: Request, res) => {
  try {
...
router.delete("/sections/:sectionId", authMiddleware, async (req: Request, res) => {
  try {
    const sectionId = String(req.params.sectionId);
